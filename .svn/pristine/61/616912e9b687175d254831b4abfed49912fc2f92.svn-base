			...
			<div class="col-lg-12">
				...
				<div class="card">
				    ...
				    <!-- 게시판 헤더 영역 끝 -->
						...
						<form id="deleteForm" action="${pageContext.request.contextPath}/account/mail/inboxdelete.do"
							method="post">
							<table class="table datatable table-hover">
								...
								<tbody>
									<c:forEach items="${messages}" var="message" varStatus="status">
										<tr>
											<td>${status.index}</td>
											<td><input type="checkbox" class="messageCheckbox"
												name="inboxMessageIndexes" value="${status.index}" /></td>
											<td><a class="d-block bk"
												href='<c:url value="/account/mail/detail.do"/>?messageIndex=${status.index}'>
													<c:out value="${fn:substring(message.from[0], 0, 40)}" />

											</a></td>
											<td><a class="d-block bk"
												href='<c:url value="/account/mail/detail.do"/>?messageIndex=${status.index}'>${empty message.subject ? '(제목 없음)' : message.subject}</a></td>
											<td><a class="d-block bk"
												href='<c:url value="/account/mail/detail.do"/>?messageIndex=${status.index}'>
												<fmt:formatDate value="${message.sentDate}" pattern="yyyy-MM-dd" /></a></td>
										</tr>
									</c:forEach>
								</tbody>
...
<script>
	document.addEventListener('DOMContentLoaded',
			function() {
				...
				const datatableSelector = document.querySelector('.datatable-selector'); // 테이블 페이지 단위 선택
				const paginationItems = document.querySelectorAll('.datatable-pagination-list-item'); // 페이징
