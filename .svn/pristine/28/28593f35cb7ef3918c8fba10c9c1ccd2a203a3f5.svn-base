...
<style type="text/css">
	...
	/* 페이징 컨트롤 페이지 번호를 가로로 나오도록 수정 */
.dataTables_paginate .pagination {
    flex-direction: row; /* 페이지 번호를 가로로 나오도록 설정 */
}
...
// 				);
				...
				// Date 객체로 변환
				var dateTime = new Date(msgData.sendTime);
				
				// 시간과 분을 가져오기
				var hours = dateTime.getHours();
				var minutes = dateTime.getMinutes();
				
				// 시간과 분을 2자리 숫자로 표시하고 ":"로 연결
...
// 					console.log("haha", msgData.sendTime);
					...
					console.log('msgData info', msgData);
					newDiv = makeMyMsg({'message': msgData.msg, 'timeString': formattedTime, 'unReadCnt': msgData.unReadCnt});
				    $('#chat').append(newDiv);
				} else {
					let newP = $('<p>').text(msgData.senderName);
					let leftMsg = makeOtherMsg({'message': msgData.msg, 'sender': msgData.senderName, 'timeString': formattedTime, 'unReadCnt': msgData.unReadCnt});
// 				    newDiv = $('<div>').append(
...
//	 			this._socket = window.opener.socket;
				...
				console.log("socket", this._socket);
	 			...
	 			this._socket.onopen = function(evt) {
	 				self._sendMessage('CHAT_ENTER', '${param.bang_id}', 'chatenter');	 			};
...
// 					webSocket.closeMessage(JSON.parse(evt.data));
					self._sendMessage('CHAT_OUT', '${param.bang_id}', 'chatout');
				}
			},
...
<!--                     <div class="mr-3"> -->
<!--                         <a href="!#"> -->
<%--                             <p class="fw-400 mb-0 text-dark-75">${chatRoom.chrNm }</p> --%>
<!--                         </a> -->
<%-- 							<h2 class="fw-400 mb-0 text-dark-75">${chatRoom.chrNm }</h2> --%>
							<h3>${chatRoom.chrNm }</h3>
...
<%-- 										<td>${crm.employee.empNo }</td> --%>
										<td>${crm.employee.department.deptNm }</td>
...
<%-- 										<td>${crm.employee.empEmail }</td> --%>
	...
	<div class="modal fade" id="inviteModal" tabindex="-1"
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h1 class="modal-title fs-5" id="exampleModalLabel">초대</h1>
					<button type="button" class="btn-close" data-bs-dismiss="modal"
						aria-label="Close"></button>
				</div>
				<div class="modal-body">
...
<%-- 										<td>${emp.empEmail }</td> --%>
										...
										<c:if test="${not memberFound}">
											<td><button class="btn btn-sm btn-outline-primary"
													onclick="fn_addMember({empNo: '${emp.empNo }', empNm: '${emp.empNm }'}, this)">추가</button></td>
										</c:if>
									</tr>
								</c:forEach>
							</tbody>
    ...
    $(document).ready(function() {
        // 페이지네이션 요소 숨기기
        $('.datatable-pagination-list').hide();
    }); 
//     $(function() {
//     	$('#datatablesSimple').DataTable();

//     	// 페이징 컨트롤 숨기기
//     	$('#datatablesSimple_paginate').hide();
//     })
    
	var chatUl = document.getElementById('chatBG');
	...
	function scrollToBottom() {
	 ...
	 makeChatList(msgListObj);
	 function makeChatList(msgList) {
		 console.log("msgLIst:: 1!!!!", msgList);
		 ...
		 msgList.sort(sortByDateAndTime);
		...
		// 정렬된 메시지를 한 번에 화면에 표시
		 ...
		 console.log("haha", msgList);
		 msgList.forEach(function (messageObj) {
		   ...
		   const unreadCnt = messageObj.unreadCnt;
...
//	 	   console.log("sender", sender);
		   ...
		   if (dateString !== currentDateString) {
			if (crmCrtTs.toDateString() === currentDate.toDateString()) {
			     chatContent.push($("<div>").addClass("text-center letter-space drop-shadow text-uppercase fs-18 w-100 mb-3").text("Today"));
			 } else {
			     chatContent.push($("<div>").addClass("text-center letter-space drop-shadow text-uppercase fs-18 w-100 mb-3").text(dateString));
			 }
		   ...
		   if (senderId == "${authMember.empNo }") {
			   ...
			   // 동적으로 생성할 요소를 jQuery로 만듭니다.
				...
				var rightChatMessage = makeMyMsg({'message': message, 'timeString': timeString, 'unReadCnt': unreadCnt});
...
//	 			if ()    
				...
				var leftMsg = makeOtherMsg({'message': message, 'timeString': timeString, 'unReadCnt': unreadCnt});
...
// 		 var sendTag = $('<p>').text(data.sender);
		if (data.unReadCnt == 0) {
			 return $('<div>', {
				  class: 'left-chat-message fs-13 mb-2 unReanCntParent',
				  html: [
				    $('<p>', {
				      class: 'mb-0 mr-3 pr-4',
				      text: data.message
...
// 		});
		...
		function fn_addMember(emp, buttonElement) {
			console.log("this", buttonElement);
			
		    buttonElement.disabled = true; // 버튼 비활성화 예시
		    buttonElement.innerText = '추가됨'; // 버튼 비활성화 예시
