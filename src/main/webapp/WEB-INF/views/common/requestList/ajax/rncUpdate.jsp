...
<body>
	...
	<div class="card">
		<div class="card-body">
			...
			<!-- Horizontal Form -->
			<form:form method="post" modelAttribute="rnc"
				action="${pageContext.request.contextPath}/infra/rncInsert.do"
				id="popupForm" enctype="multipart/form-data">
					...
					<div class="col-sm-8" style="margin-bottom: 10px;">
						<c:forEach items="${commonList}" var="item">
							<c:if test="${item.codeId == 'REF' || item.codeId == 'REC'}">
								<input type="radio" name="rrType" value="${item.codeId}"
									id="category${item.codeId}" />
				...
				<div class="row mb-3" style="margin-bottom: 10px;">
					<label for="inputEmail3" class="col-sm-3 col-form-label">요청내용</label>
					<div class="col-sm-8">
							...
							<c:forEach items="${zoneList}" var="zone" varStatus="status">
								<c:if
									test="${status.index == 0 || zoneList[status.index - 1].zoneFloor != zone.zoneFloor}">
									<option value="${zone.zoneFloor}" label="${zone.zoneFloor}" />
								</c:if>
							</c:forEach>
						</select>

						<form:select class="form-select" type="text" path="rrLoc" 
							id="zoneSelect" >
				...
				<div class="row mb-3">
					<label for="inputEmail3" class="col-sm-3 col-form-label">발생일시</label>
					<div class="col-sm-8">
						<input type="datetime-local" id="rrOccurTime" name="rrOccurTs"
							class="form-control" />
					</div>
				</div>

				<div class="row mb-3">
					<label for="inputEmail3" class="col-sm-3 col-form-label">첨부파일</label>
					<div class="col-sm-8">
						<input type="file" name="flFiles" class="form-control-file"
							multiple />
					</div>
				</div>
				...
				<div class="text-center">
					<button type="submit" class="btn btn-primary"
						onclick="submitPopupForm()">수정</button>
