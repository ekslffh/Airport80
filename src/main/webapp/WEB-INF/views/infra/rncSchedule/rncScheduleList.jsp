						...
						<nav>
							...
							<ul class="nav nav-pills mb-3" id="sttsNav" role="tablist">
								...
								<li class="nav-item" role="presentation">
									<button
										...
										id="dlStts" data-bs-toggle="pill" data-bs-target="#pills-dl"
										type="button" role="tab" aria-controls="pills-dl"
										aria-selected="false">진행중(지연)</button>
								</li>
								<li class="nav-item" role="presentation">
									<button
										class="nav-link ${paging.simpleCondition.tab == 3 ? 'active show' : ''}"
										id="woStts" data-bs-toggle="pill" data-bs-target="#pills-wo"
										type="button" role="tab" aria-controls="pills-wo"
										aria-selected="false">외주</button>
								</li>
								<li class="nav-item" role="presentation">
									<button
										class="nav-link ${paging.simpleCondition.tab == 4 ? 'active show' : ''}"
							...
							<div class="tab-content pt-2" id="pills-tab-content">
								...
								<div
									...
									id="pills-wt" role="tabpanel" aria-labelledby="home-tab">
									<table class="table Default Table">
										...
										<tbody id="table1">
											<c:set var="wtList" value="${paging.dataList }" />
											...
											<c:if test="${not empty wtList}">
												<c:forEach items="${wtList }" var="wt">
													...
													<tr class="clickable-row" data-rsid="${wt.rsId}">
														<td>${wt.rsId }</td>
														<td>${wt.zone.zoneFloor }층${wt.zone.zoneCtgr }</td>
														<td>${wt.rsContent }</td>
								...
								<div
									...
									id="pills-pg" role="tabpanel" aria-labelledby="home-tab">
									<table class="table Default Table">
										<thead class="table-light">
											<tr>
												<th  >일정ID</th>
								...
								<div
									...
									id="pills-dl" role="tabpanel" aria-labelledby="home-tab">
									<table class="table Default Table">
										<thead class="table-light">
											<tr>
												<th  >일정ID</th>
										...
										<tbody id="table3">
											<!-- 									<div class="req-body thtble" style="height: 600px;"> -->
											<!-- 										<table class="table Default Table"> -->
											<c:set var="dlList" value="${paging.dataList }" />
											<c:if test="${empty dlList }">
												<tr>
													<td colspan="10">등록된 일정 없음.</td>
												</tr>
											</c:if>
											<c:if test="${not empty dlList}">
												<c:forEach items="${dlList }" var="dl">
													...
													<tr class="clickable-row" data-rsid="${dl.rsId}">
														<td>${dl.rsId }</td>
														<td>${dl.zone.zoneFloor }층${dl.zone.zoneCtgr }</td>
														...
														<td>${dl.employee.empNm }</td>
														<td>${dl.common.codeNm }</td>
														<td>
															<button class="btn btn-primary">처리완료</button>
														</td>
													</tr>
								...
								<!-- 외주 목록탭 -->
								<div
									class="tab-pane fade   ${paging.simpleCondition.tab == 3 ? 'active show' : ''}"
									id="pills-wo" role="tabpanel" aria-labelledby="home-tab">
									<table class="table Default Table">
										<thead class="table-light">
											<tr>
												<th>일정ID</th>
										...
										<tbody id="table4">
											<c:set var="woList" value="${paging.dataList }" />
											<c:if test="${empty woList }">
												<tr>
													<td colspan="10">등록된 일정 없음.</td>
												</tr>
											</c:if>
											<c:if test="${not empty woList}">
												<c:forEach items="${woList }" var="wo">
													<%-- 										<input type="hidden" value="${rncReq.rrType}"/> --%>
													<tr class="clickable-row" data-rsid="${wo.rsId}">
														<td>${wo.rsId }</td>
														<td>${wo.zone.zoneFloor }층${wo.zone.zoneCtgr }</td>
														...
														<td>${wo.employee.empNm }</td>
														<td>${wo.common.codeNm }</td>
														<td>
															<button class="btn btn-primary">처리완료</button>
														</td>
													</tr>
								...
								<!-- 처리완료 목록탭 -->
								<div
									class="tab-pane fade   ${paging.simpleCondition.tab == 4 ? 'active show' : ''}"
									id="pills-pc" role="tabpanel" aria-labelledby="home-tab">
									<table class="table Default Table">
										<thead class="table-light">
											<tr>
												<th  >일정ID</th>
...
<script>
	...
	function handleTabClick(tabId) {
		...
		case "dlStts":
			...
			console.log("진행중 탭을 클릭했습니다.");
			$(tab1Form).find("[name=tab]").val(2);
			break;
		case "woStts":
			// 전체조회 탭을 클릭했을 때 수행할 작업
			// 예: 전체 데이터를 로드하거나 다른 동작 수행
			console.log("외주 탭을 클릭했습니다.");
			$(tab1Form).find("[name=tab]").val(3);
			break;
		case "pcStts":
			// 전체조회 탭을 클릭했을 때 수행할 작업
			// 예: 전체 데이터를 로드하거나 다른 동작 수행
			console.log("처리완료 탭을 클릭했습니다.");
			$(tab1Form).find("[name=tab]").val(4);
			break;
		default:
	...
	document.getElementById("dlStts").addEventListener("click", function() {
		handleTabClick("dlStts");
	});
	document.getElementById("woStts").addEventListener("click", function() {
		handleTabClick("woStts");
	});
